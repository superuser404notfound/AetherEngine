...
### Coming Soon
- **Audio output** — AVSampleBufferAudioRenderer with multichannel support (Phase 2)
- **A/V synchronization** — PTS-based sync clock (Phase 2)
- **Seeking** — keyframe-accurate with buffer flush (Phase 3)
...
├── Audio/
│   └── (Phase 2)               AVSampleBufferAudioRenderer
├── Sync/
│   ├── FrameQueue.swift        Thread-safe decoded frame buffer
│   └── (Phase 2)               PTS-based A/V sync clock
├── Subtitles/
...
```
URL → FFmpeg demuxer → AVPackets → VideoToolbox HW decoder
    → CVPixelBuffer → FrameQueue → CADisplayLink
    → Metal renderer → CAMetalLayer → Screen
```
