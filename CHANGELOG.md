...
- **PGS subtitles blank (and cycling while paused) after an audio-track switch (#112).** An audio switch does a `keepNativeHost` reload. `stopInternal` resets `playlistShiftSeconds` to 0, and on a disc the new producer republishes its shift asynchronously (the reload lands paused / `waitingToPlay`, so no clock tick has folded it back yet), so the subtitle re-arm's live `sourceTime` read had collapsed to the playlist axis (equal to `resumeAt`), roughly the producer shift behind the true source PTS. On a remote Blu-ray the device log showed it exactly: playhead at source PTS ~1603.7 s, but the re-armed PGS side demuxer anchored at 1003.8 s, ~600 s behind. It reconstructed a region the playhead had long passed (never covering it, so nothing showed) and crawled forward publishing stale open-ended acquisition-point cues that replaced each other on screen even though the video was paused. Because an audio switch does not move the playhead, the reload now snapshots `sourceTime` before `stopInternal` (where the old shift is still live and the value is the real source PTS) and re-arms both subtitle channels with that anchor instead of a post-reset live read. Fast-forward and subtitle-track switches were unaffected (they carry a correct source-PTS anchor) and already worked.
- **AVAudioSession hang-risk diagnostic from the engine constructor (#114).** `AetherEngine.init()` runs on the main actor, so its `setCategory(.playback, ...)` / `setSupportsMultichannelContent(true)` pair, both XPC round-trips to mediaserverd, executed on the main thread. When the shared session was already active (a second playback in the same app session, or a live route like AirPods), that round-trip could block long enough to trip Xcode's runtime hang-risk diagnostic and risk the watchdog. The category is now declared on a detached task; every load path awaits it before the first activation, so the blocking call leaves the main thread while issue #24's "declare early, never activate at init" contract is preserved (nothing reads the category synchronously at init).
- **PGS subtitles blank for tens of seconds after a seek or audio-track switch, reworked (#112).** PGS (Blu-ray) subtitles are stateful and sparse: a line stays on screen from its composition until the next one clears it, often minutes later. Earlier rounds reconstructed the active line on every seek by seeking back a fixed lead-in and decoding forward. That fixed window is a dead-end: the distance back to the active line's defining composition is unbounded, so a small window (24 s on disc) missed lines composed earlier while a large one (60 s) re-downloaded huge spans on a remote index-less disc and was superseded before it drew a cue. This rework replaces the per-seek back-scan with three parts. (1) **Retained-store seeks:** the engine already keeps ~300 s of decoded cues, so a backward / in-region seek whose target the store already covers now shows the active line instantly with zero I/O and no re-download (the common "re-watch that line" case), instead of clearing the store and reconstructing; coverage is derived from the store (target at/below the decoded frontier AND inside the newest covering cue, so a held/dropped-cue gap still reconstructs). (2) **Audio-track switch preserve:** a switch does not move the playhead, so the visible bitmap line is snapshotted before the pipeline reload and restored after, keeping it on screen instead of reconstructing it from scratch. (3) **Acquisition-point-aware reconstruction:** for a genuine forward jump into not-yet-decoded territory the reader still seeks back and reconstructs, but now reads the PGS `composition_state`, so a self-contained Acquisition Point / Epoch Start covering the playhead publishes immediately rather than waiting for a successor trim (confined to the reconstruction pass, so the #100 catch-up flash stays suppressed). A forward jump into never-watched, sparse-dialogue territory on a non-indexed remote disc can still show a brief gap until the next composition, the same as mpv / VLC: without an index the active line cannot be reached there without re-downloading.
